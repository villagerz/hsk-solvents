...
isEscaped :: Char -> Parser Char
isEscaped c = case toSpecialCharacter c of
               Empty -> unexpectedCharParser c
               Full c' -> valueParser . fromSpecialCharacter $ c'

escaped :: Parser Char
escaped = isEscaped =<< character
...
jsonValue' = (const JsonTrue <$> jsonTrue) |||
...
jsonObject ::
  Parser Assoc
jsonObject = betweenSepbyComma '{' '}' keyValParser

keyValParser :: Parser (Chars, JsonValue)
keyValParser = (,) <$> (jsonString <* spaces <* charTok ':') <*> jsonValue
...
readJsonValue f = do
  jsonStr <- readFile f
  return (parse jsonValue jsonStr)
