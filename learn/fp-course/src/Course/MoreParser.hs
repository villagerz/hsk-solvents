...
-- analysing the type signature since the hint was to use traverse
-- travserse :: (a -> f b) -> t a -> f ( t b )
--  travserse :: (a -> f b) -> t a -> Parser ( List  Char ) => f = Parser, t = List, b = Char
--  travserse :: (a -> Parser Char) -> List a -> Parser ( List Char )
--  travserse :: (Char -> f b) -> t a -> f ( t b )
...
hex ::
  Parser Char
hex = (tochar . readHex) <$> thisMany 4 (satisfy isHexDigit) 
  where tochar Empty = chr 0
        tochar (Full i) = chr i
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll  ps = satisfy (\c -> and (sequence ps c))
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ps = satisfy (\c -> or (sequence ps c))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r a = betweenCharTok l r (sepby a commaTok)
