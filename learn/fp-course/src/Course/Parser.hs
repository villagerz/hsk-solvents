...
character ::
  Parser Char
character = P trychar
  where trychar Nil = UnexpectedEof
        trychar (c:.cs) = Result cs c
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) f (P pf) = P (\i -> (onResult (pf i) go))
                   where go i a = Result i (f a)
...
(|||) ::
  ...
  -> Parser a
(|||) f s = P (\i ->
                          (let result = parse f i
                           in case isErrorResult result of
                                True -> parse s i
                                False -> result))
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f (P p) = P (\i -> (onResult (p i) next))
    where next i a = parse (f a) i
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) f a =  f >>= \f' -> a >>= \a' -> pure (f' a') -- (go' f) =<< a

go' :: Parser (a -> b) -> a -> Parser b
go' f a = P (\i -> onResult (parse f i) k)
  where k i' f' = Result i' (f' a)
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy check =  (\c -> if check c then valueParser c else unexpectedCharParser c) =<< character
...
sequenceParser (p :. ps) = (:.) <$> p <*> (sequenceParser ps) -- this can be done simply using Applicative.sequence
...
surnameParser ::
  Parser Chars
surnameParser = makename <$> upper <*> (thisMany 5 lower) <*> (list lower)
  where makename c n5 r = (c :. n5 ) ++ r
...
smokerParser ::
  Parser Bool
smokerParser = (is 'y') ||| (is 'n') >>= smokep -- /<$/ as in /True <$ is 'y' / in Functor is also useful
  where smokep c = valueParser (c == 'y')
