...
convertInteractive ::
  IO ()
convertInteractive = vooid( putStrLn "Enter a string:" >-
                            getLine >>= \s ->
                            putStr ("You entered [" ++ s ++ "].toUpper = ") >-
                            putStrLn (toUpper <$> s)
                          )
...
reverseInteractive = do
  ...
  fname <- getLine
  fstr <- readFile fname
  putStrLn ("File read. " ++ listh (show (length fstr)) ++ " characters.")
  putStrLn "Name of file to write reversed content to:"
  oName <- getLine
  writeFile oName (reverse fstr)
  putStrLn "Done."
...
encodeInteractive = do
  ...
  s <- getLine
  putStrLn (urlencode s)
  

urlencode :: Chars -> Chars
urlencode s = join (urlify <$> s)
  where urlify c = case c of
                     ' ' -> "%20"
                     '\t' -> "%09"
                     '\"' -> "%22"
                     _ -> (c:.Nil)
